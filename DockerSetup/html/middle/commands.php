...
{
    global $backend_url;
    $fetch_q_b = curl_request_input($backend_url . "auto_grader.php", $data);
    return $fetch_q_b;
}
...
{    
    ...
    $file = fopen("student_submission.py", 'w');
    if ( !$file ) {
    ...
    $cmd = 'python3 student_submission.py 2>&1';
    $python_output = exec($cmd);
    //echo var_dump($python_output);
    //Delete the file
    unlink('student_submission.py');
