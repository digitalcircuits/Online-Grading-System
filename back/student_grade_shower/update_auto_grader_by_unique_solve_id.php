...
global $con;
$sql = "UPDATE `student_ans_exam` SET `auto_grader_comments` = '$POSTupdate_grade' WHERE `student_ans_exam`.`uniq_exam_solve_id` = $POSTunique_id;";
...
elseif(empty($POSTupdate_grade))
{
    $errk = array(
        'return_code' => 0,
        'error_message' => 'You sent an empty "update grade" for the auto grader',
...
else
{
    ...
    if ($results)
    {
        $stat = array(
            'return_code' => 1,
            'message' => 'Success! Auto Grader Grades Grades have bee updated!',
            'update_grade' => $POSTupdate_grade
        );
